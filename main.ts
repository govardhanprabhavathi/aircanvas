...
class AirCanvas {
  ...
  private lastTwoFingersPosition: { x: number; y: number } | null = null;
  private lastThreeFingersPosition: { x: number; y: number } | null = null;
  private bgOffsetX = 0;
  private bgOffsetY = 0;
  ...
  private setupButtonListeners(): void {
    ...
    recenterBtn?.addEventListener('click', () => {
      this.drawingCanvas.resetPanAndZoom();
      ...
      if (appContainer) {
        appContainer.style.backgroundPosition = '0px 0px';
        appContainer.style.backgroundSize = '15px 15px';
      }
    });
  ...
  private handleGesture(state: GestureState, landmarks: HandLandmarks): void {
    ...
    switch (state.current) {
      ...
      case 'four_fingers':
        this.handleFourFingers(landmarks);
        break;

      case 'two_fingers':
        this.handleTwoFingers(landmarks);
        break;

      case 'three_fingers':
        this.handleThreeFingers(landmarks);
        break;
      ...
      default:
        ...
        this.lastTwoFingersPosition = null;
        this.lastThreeFingersPosition = null;
        break;
    }
  ...
  private handleFourFingers(landmarks: HandLandmarks): void {
    const center = this.gestureDetector.getPalmCenter(landmarks);
    
    // Check if there's an object to delete
    let hitObject = this.grabbedObject;
    if (!hitObject) {
      hitObject = this.objectManager.getObjectAtPosition(center.x, center.y);
    }
  ...
  private handleTwoFingers(landmarks: HandLandmarks): void {
    const indexTip = this.gestureDetector.getIndexTip(landmarks);

    if (!this.lastTwoFingersPosition) {
      this.lastTwoFingersPosition = indexTip;
      return;
    }

    // Calculate hand movement
    const deltaX = indexTip.x - this.lastTwoFingersPosition.x;
    const deltaY = indexTip.y - this.lastTwoFingersPosition.y;

    // Pan 2D Canvas strokes
    this.drawingCanvas.pan(deltaX, deltaY);

    // Pan 3D Camera to shift 3D objects
    this.scene3D.panCamera(deltaX, deltaY);

    // Pan CSS Background for infinite canvas illusion
    this.bgOffsetX += deltaX;
    this.bgOffsetY += deltaY;
    
    const appElement = document.getElementById('app');
    if (appElement) {
      appElement.style.backgroundPosition = `${this.bgOffsetX}px ${this.bgOffsetY}px`;
    }

    this.lastTwoFingersPosition = indexTip;
  }

  private handleThreeFingers(landmarks: HandLandmarks): void {
    const indexTip = this.gestureDetector.getIndexTip(landmarks);

    if (!this.lastThreeFingersPosition) {
      this.lastThreeFingersPosition = indexTip;
      return;
    }

    const deltaY = indexTip.y - this.lastThreeFingersPosition.y;
    
    // Negative deltaY means hand moved UP (zoom in)
    // Positive deltaY means hand moved DOWN (zoom out)
    const zoomSpeed = -0.01;
    const zoomDelta = deltaY * zoomSpeed;

    // 2D Zoom
    this.drawingCanvas.zoom(zoomDelta);

    // 3D Zoom
    this.scene3D.zoomCamera(zoomDelta);

    // Update CSS background size for infinite zoom effect
    const appElement = document.getElementById('app');
    if (appElement) {
      const currentZoom = this.drawingCanvas.getZoomLevel();
      appElement.style.backgroundSize = `${15 * currentZoom}px ${15 * currentZoom}px`;
    }
  ...
  private async createBalloon(stroke: Stroke): Promise<void> {
    ...
    try {
      // Calculate stroke center in screen coordinates
      const centerWorld = stroke.points.reduce(
        (acc, p) => ({ x: acc.x + p.x, y: acc.y + p.y }),
        { x: 0, y: 0 }
      );
      centerWorld.x /= stroke.points.length;
      centerWorld.y /= stroke.points.length;
      
      const screenPos = this.drawingCanvas.toScreen(centerWorld);

      await this.objectManager.createFromStroke(stroke, screenPos);
