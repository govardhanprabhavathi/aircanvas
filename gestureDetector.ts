...
export class GestureDetector {
  ...
  getPalmCenter(landmarks: HandLandmarks): Point2D {
  ...
  private detectGestureType(landmarks: HandLandmarks): GestureType {
    ...
    // 1. PINCH CHECK (High Priority) - Threshold increased to 80 for easier grabbing
    const pinchDistance = this.distance(lm[LANDMARKS.THUMB_TIP], lm[LANDMARKS.INDEX_TIP]);
    
    if (pinchDistance < 80) { // <--- INCREASED THRESHOLD (Easier to pinch/select)
      return 'pinch';
    }
    ...
    // 3. TWO FINGERS CHECK (Index + Middle extended)
    if (this.isTwoFingers(landmarks)) {
      return 'two_fingers';
    }

    // 4. THREE FINGERS CHECK (Index, Middle, Ring extended)
    if (this.isThreeFingers(landmarks)) {
    ...
    // 4. FOUR FINGERS CHECK (Index, Middle, Ring, Pinky extended)
    if (this.isFourFingers(landmarks)) {
      return 'four_fingers';
    }
  ...
  private isTwoFingers(landmarks: HandLandmarks): boolean {
    // Index and Middle OPEN
    const indexOpen = this.isFingerExtended(landmarks, LANDMARKS.INDEX_TIP, LANDMARKS.INDEX_PIP);
    const middleOpen = this.isFingerExtended(landmarks, LANDMARKS.MIDDLE_TIP, LANDMARKS.MIDDLE_PIP);
    // Ring, Pinky CLOSED
    const ringClosed = !this.isFingerExtended(landmarks, LANDMARKS.RING_TIP, LANDMARKS.RING_PIP);
    const pinkyClosed = !this.isFingerExtended(landmarks, LANDMARKS.PINKY_TIP, LANDMARKS.PINKY_PIP);
  ...
  private isThreeFingers(landmarks: HandLandmarks): boolean {
    const indexOpen = this.isFingerExtended(landmarks, LANDMARKS.INDEX_TIP, LANDMARKS.INDEX_PIP);
    const middleOpen = this.isFingerExtended(landmarks, LANDMARKS.MIDDLE_TIP, LANDMARKS.MIDDLE_PIP);
    const ringOpen = this.isFingerExtended(landmarks, LANDMARKS.RING_TIP, LANDMARKS.RING_PIP);
    const pinkyClosed = !this.isFingerExtended(landmarks, LANDMARKS.PINKY_TIP, LANDMARKS.PINKY_PIP);
    
    return indexOpen && middleOpen && ringOpen && pinkyClosed;
  }

  private isFourFingers(landmarks: HandLandmarks): boolean {
    const indexOpen = this.isFingerExtended(landmarks, LANDMARKS.INDEX_TIP, LANDMARKS.INDEX_PIP);
    const middleOpen = this.isFingerExtended(landmarks, LANDMARKS.MIDDLE_TIP, LANDMARKS.MIDDLE_PIP);
    const ringOpen = this.isFingerExtended(landmarks, LANDMARKS.RING_TIP, LANDMARKS.RING_PIP);
    const pinkyOpen = this.isFingerExtended(landmarks, LANDMARKS.PINKY_TIP, LANDMARKS.PINKY_PIP);
    
    // Note: thumb can be either open or closed for a four-finger gesture, but usually it's tucked in.
