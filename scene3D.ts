...
export class Scene3D {
  ...
  zoomCamera(delta: number): void {
    // Negative to get closer when delta is positive (zooming in)
    this.cameraDistance -= delta * 15;
  ...
  panCamera(deltaX: number, deltaY: number): void {
    ...
    // Scale pixel delta to world units based on camera distance
    const panSpeed = 0.01 * (this.cameraDistance / 10);
    
    // Reverse directions so it feels like dragging the canvas
