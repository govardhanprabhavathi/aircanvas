...
export class ObjectManager {
  ...
  private findBalancedPosition(obj: BalloonObject): void {
    ...
    for (const other of this.objects) {
      ...
      if (dist < SCENE.COLLISION_RADIUS) {
        ...
        diff.normalize().multiplyScalar(pushForce * 0.1); // Reduced pushing force
        newPos.add(diff);
      }
    }
  ...
  async clearAll(): Promise<void> {
    ...
    for (let i = 0; i < objectsToRemove.length; i++) {
      const delay = i * 0.05; // Slightly faster staggering
      promises.push(
        new Promise((resolve) => {
          gsap.delayedCall(delay, async () => {
            await this.removeObject(objectsToRemove[i], true);
            resolve();
          });
        })
      );
