...
export class DrawingCanvas {
  ...
  startStroke(point: Point2D, color: string): void {
    ...
    this.livePosition = worldPoint;
    this.filteredPosition = worldPoint;
    this.recentPoints = [worldPoint];
  }
  ...
  addPoint(point: Point2D): void {
    ...
    const filtered = this.applyJitterFilter(worldPoint);
  ...
  toWorld(point: Point2D): Point2D {
    ...
    return {
      x: (point.x - centerX) / this.zoomLevel + centerX - this.panOffset.x,
      y: (point.y - centerY) / this.zoomLevel + centerY - this.panOffset.y
    };
  }
  ...
  toScreen(point: Point2D): Point2D {
    ...
    return {
      x: (point.x + this.panOffset.x - centerX) * this.zoomLevel + centerX,
      y: (point.y + this.panOffset.y - centerY) * this.zoomLevel + centerY
    };
  }
